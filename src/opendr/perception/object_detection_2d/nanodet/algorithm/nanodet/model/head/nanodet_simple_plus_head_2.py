...
class SimplifierNanoDetPlusHead_2(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(SimplifierNanoDetPlusHead_2, self).__init__()
    ...
    def _init_layers(self):
        ...
        self.cls_convs2 = self._buid_not_shared_head()
        ...
        self.gfl_cls2 = nn.Conv2d(self.feat_channels, self.num_classes + 4 * (self.reg_max + 1), 1, padding=0)

    def _buid_not_shared_head(self):
        cls_convs = nn.Sequential(
            self.ConvModule(self.in_channels, self.feat_channels, k=self.kernel_size, s=1, p=self.kernel_size // 2,
                            act=act_layers(self.activation)),
            self.ConvModule(self.feat_channels, self.feat_channels, k=self.kernel_size, s=1, p=self.kernel_size // 2,
                            act=act_layers(self.activation))
        )
        return cls_convs
    ...
    def init_weights(self):
        ...
        for m in self.cls_convs1.modules():
            if isinstance(m, nn.Conv2d):
                normal_init(m, std=0.01)
        for m in self.cls_convs2.modules():
            if isinstance(m, nn.Conv2d):
                normal_init(m, std=0.01)
        ...
        normal_init(self.gfl_cls2, std=0.01, bias=bias_cls)
        print("Finish initialize NanoDet-Plus Head.")

    @torch.jit.unused
    def forward(self, feats: List[Tensor]):
        x0 = feats[0]
        x0 = self.cls_convs0(x0)
        x0 = self.gfl_cls0(x0).flatten(start_dim=2)

        x1 = feats[1]
        x1 = self.cls_convs1(x1)
        x1 = self.gfl_cls1(x1).flatten(start_dim=2)

        x2 = feats[2]
        x2 = self.cls_convs2(x2)
        x2 = self.gfl_cls2(x2).flatten(start_dim=2)
        outputs = torch.cat((x0, x1, x2), dim=2).permute(0, 2, 1).contiguous()
        return outputs
...
if __name__ == '__main__':
    ...
    model = SimplifierNanoDetPlusHead_2(
        **head_cfg
    ).eval()
