...
class SimplifierNanoDetPlusHead(nn.Module):
    ...
    def _init_layers(self):
        ...
        for stem in range(self.stems):
            conv_stem = self._buid_not_shared_head()
            conv_stem.i = self.stems + (3 * stem)
            conv_stem.f = stem
            self.head.add_module(f"conv_stems_{stem}", conv_stem)

            gfl_conv = nn.Conv2d(self.feat_channels, self.num_classes + 4 * (self.reg_max + 1), 1, padding=0)
            gfl_conv.i = (self.stems + 1) + (3 * stem)
            gfl_conv.f = -1
            self.head.add_module(f"dw_{stem}", gfl_conv)

            flatten = Flatten(start_dim=2)
            flatten.i = (self.stems + 2) + (3 * stem)
            flatten.f = -1
            self.head.add_module(f"flatten_{stem}", flatten)

            self.out_stages.append((self.stems + 2) + (3 * stem))

        concat = Concat(2)
        concat.i = -1
        concat.f = self.out_stages
        self.head.add_module("concat", concat)
    ...
    def init_weights(self):
        ...
        for m in self.head.named_modules():
            if "dw" in m[0]:
                bias_cls = -4.595
                normal_init(m[1], std=0.01, bias=bias_cls)
                print("")
...
if __name__ == '__main__':
    ...
    model = SimplifierNanoDetPlusHead(
        ...
        quant=False,
        loss=DotDict(dict(
            loss_qfl=DotDict({"name": "QualityFocalLoss", "use_sigmoid": True, "beta": 2.0, "loss_weight": 1.0}),
            loss_dfl=DotDict({"name": "DistributionFocalLoss", "loss_weight": 0.25}),
            loss_bbox=DotDict({"name": "GIoULoss", "loss_weight": 2.0})
        ))
    ).eval()
    imgszs = [(1, 256, 136, 240), (1, 256, 68, 120), (1, 256, 34, 60)]
    __dumy_input = [torch.empty(*imgsz, dtype=torch.float, device="cpu") for imgsz in imgszs]

    writer = SummaryWriter(f'./models/nanodet_plus_head')
