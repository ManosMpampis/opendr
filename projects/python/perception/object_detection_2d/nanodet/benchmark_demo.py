...
if __name__ == '__main__':
    ...
    parser.add_argument("--model", help="Model for which a config file will be used", type=str, default="8_0_0_0_0_0_8")
    parser.add_argument("--optimize-jit", help="", action="store_false")
    parser.add_argument("--optimize-onnx", help="", action="store_true")
    ...
    nanodet.cfg.defrost()
    temp_name = nanodet.cfg.check_point_name
    nanodet.cfg.check_point_name = "model_state_best"
    nanodet.cfg.data.val.input_size = [1920, 1088]  # [w,h]
    nanodet.cfg.freeze()
    # nanodet.load(save_path, verbose=True)
    nanodet.cfg.defrost()
    nanodet.cfg.check_point_name = temp_name
    nanodet.cfg.freeze()
